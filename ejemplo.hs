...
Para implementar este sistema nos enviaron las siguientes especificaciones y nos pidieron que hagamos el desarrollo enteramente en Haskell,
 utilizando los tipos requeridos y solamente las funciones que se ven en la materia Introducción a la Programación / Algoritmos y Estructuras de Datos I 
 (FCEyN-UBA).
...
calcularAprobacion::[Int]->Int->Int->Bool
calcularAprobacion [] umbral contador | contador > umbral = True
                                      | otherwise = False
calcularAprobacion (nota:notas) umbral contador | nota >= 4 = calcularAprobacion notas umbral (contador+1)
                                                | otherwise = calcularAprobacion notas umbral (contador)

aproboMasDeNMaterias2::[([Char],[Int])]->[Char]->Int->Bool
aproboMasDeNMaterias2 (alumno:alumnos) elegido umbral | fst alumno == elegido = calcularAprobacion (snd alumno) umbral 0
                                                      | otherwise = aproboMasDeNMaterias2 alumnos elegido umbral
>>>>>>> Stashed changes
